...
> 「什么东西保证这个项目没坏」的清单。约束本身在 [`design/`](design/) 与 `AGENTS.md` 第 5 节，本文只说**怎么验**。
...
**判据是「这个 fixture 决定不决定解析器的结构」，不是「它算不算边界情况」。** 下面这些各自钉住一个只能靠它证伪的形态。**右列只回答「它照红的是哪条前提、哪种写法」，机制与实测数字归 [`decisions.md`](decisions.md)**——写进来不报错，只是同一件事在两处各长一份，而改的时候只会想起一处。

| fixture | 它唯一能证伪的东西 |
|---|---|
| 路径含非 ASCII / 空格 / 引号 | `-z` 与 `core.quotePath=false` 有没有真的生效 |
| 重命名（含相似度阈值边界，一个 add 一个不 add——**阈值之下那一半必须一并入 index**，不入时 `git mv` 后重写内容 git 照报 `R100`） | 解析循环是有状态还是无状态平铺 |
