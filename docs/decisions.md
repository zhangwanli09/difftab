...
>
> **按域分节，每节末尾自带本域那张「被排除的做法」分表。**`design/` 一律以锚点链进对应小节——任一个域只占全文的十分之一到五分之一。**但读取的最小单位是文件，锚点只保证一眼落到那一节、并不减少读进来的量**，因此每节必须能被单独读懂，别写成前后互相参照的长链。新增内容先认域，别在文件末尾另起一摊。
>
...
- **Windows 的突发写入会溢出通知缓冲区，漏出无 `filename` 的事件**（CI 三平台 × Node 22.0.x / 24 / 26）：间隔 250ms（> 合并窗口 150ms）写 6 个文件，**三端九档一律 0 次刷新**；一口气写 50 个，则 **windows × Node 24 / 26 各出 1 次**，macOS 与 Linux 仍是 0。**后来在 windows × Node 26 上实测到 2 次**——那个「1 次」不是上界，只是当时的采样。
...
| 用「一口气写 50 个文件」那一路来验过滤（曾经有，已删） | **在能断言的地方冗余，在不冗余的地方断言不了。** macOS / Linux 上合并窗口把整批压成**一个**事件，过滤失效时它只漏出 1 个而间隔写入逐个漏出 6 个——它是同一条断言的更弱形式，没有独有的覆盖；Windows 上「缓冲区溢出」与「过滤失效」给出同一个信号，只能带一个放宽的上限，而那个上限正是它唯一稳定的产出（实测溢出过 2 次，超过原定的 1）。调大上限只是让它更晚一点再骗人一次 |
...
- **退出前的提示走 `fs.writeSync(2, …)`，且必须容许这次写失败**：**管道写在 Windows 上是异步的**、POSIX 上是同步的，于是读端一走（`difftab --no-open | head -1`）EPIPE 以一个 `'error'` 事件到达，零监听器的流收到它就是整个进程带裸栈以 1 退出。**macOS 上这个码不总是 EPIPE**：Node 的 stdio 管道在那里是 unix socket，对端刚关闭时内核有个竞态回 EPROTOTYPE，libuv 的 `uv__try_write` 把它翻译成 ECONNRESET（源码里 `__APPLE__` 那段注释写明是 10.10–10.15 的内核 bug）。实测：2026-09-19 `main` 上一个纯文档提交的 CI 在 `smoke macos-latest · Node 26` 红了一次——`| head -1` 那条 98ms 就以 1 退出，重跑即绿；本机 Darwin 27 上 Node 24 / 26 各 300 次全是 EPIPE（其中 `'error'` 事件 298 次、`writeSync` 同步抛 2 次），复现不了。监听器因此咽 EPIPE 与 ECONNRESET 两个码，别的照抛。另一半是 `writeSync` 自己：读端已关闭时它**抛** EPIPE，在 45 秒空闲退出的定时器回调里抛出时退出闩已经合上、`server.close()` 不再执行，进程带着一屏 Node 栈以 **1** 退出，而这条路承诺的是干净的 **0**。
...
- **`synchronisedScroll` 是并排两侧唯一的联动机制，且没有回声抑制**（就 3.4.56 的 `diff2html-ui-base` 源码逐行核对）：`draw()` 里按 `.d2h-file-wrapper` 取出那对 `.d2h-file-side-diff`，两侧的 `scroll` 事件**无条件**互相回写 `scrollLeft` / `scrollTop`——回写引起的那次事件与用户自己滚出来的那次分不开。两侧滚动上限由各自最长行决定、通常不等，于是越过窄侧上限的那一下，窄侧被夹住后仍冒出一次事件，把宽侧回写到窄侧的上限：**表现为边界处顿一下，不是卡死**，之后继续正常滚（Chrome 实测，拿本仓库对 `docs/decisions.md` 的工作区改动、视口 1600 走并排：两侧横向上限分别是 **3765 / 4839**；把宽的那侧写到 4200，两侧一起落在 3765，再写 4500 则停在 **3765 / 4500**——窄侧饱和之后不再冒事件，宽侧照常滚到底）。上限为 0 的那一侧（新增文件的左半边全是空占位）反而不受影响——写不动就不产生事件。CSS 那侧没有替代品：`.d2h-file-side-diff{overflow-x:scroll;overflow-y:hidden;width:50%}` 让两半各自成为滚动容器，而把滚动收到共同祖先上得先给这两半 `overflow-x: visible`——两半各占 50% 宽，溢出的正文不再被裁，随即互相压盖。
...
| 文件内容构造成「全新增 diff」交给 diff2html 渲染 | 省下一个渲染路径，代价是每一行前面都挂着 `+`、整片底色是新增绿——读起来像在审一份补丁而不是读一个文件，而「看项目整体面貌」正是要摆脱补丁视角 |
| 文件视图的行号靠按行切分高亮输出 | hljs 的 span 跨行，切开要自己重开标签补齐——那正是上面「自行重写 diff2html 高亮切分逻辑」被排除的同一件事，换个地方做不会变得更容易。兄弟元素做行号槽只要两个 `<pre>` 同 `line-height` |
| 给「显示被忽略的文件」做一个开关 | 换来的是一个多数人只会拨一次的按钮，外加一档状态、一份端点参数与一组测试；灰显本身已经把「这不是你的代码」说清楚了 |
...
- **侧栏那条 `border-r` 与最左那列行号的左边框会叠成一条 2px**（2026-08-30 对一张侧栏右边界附近的页面截图逐像素解出，DPR=1，坐标相对裁剪原点）：分隔线处 `x=40` 与 `x=41` **两列**都是 `#e1e1e1`（`--color-panel-border` 的 `#e5e5e5` 经截图色彩管理后的取值），而补丁表格结束之后只剩 `x=40` 一列、`x=41` 退回底色——**同一条分隔线上粗下细**，这正是「看着像两条边框」的来源，不是某处写了 2px。上面那条的归属也已坐实：`x=41`…`x=92` 共 **52 px** = `4em @ 13px`，与 `.d2h-code-side-linenumber` 的 border-box 宽度（`width:4em` + `box-sizing:border-box`）分毫不差。两种版式都中招——`.d2h-code-linenumber` 与 `.d2h-code-side-linenumber` 都是 `border-width:0 1px`。修法见 [`design/style.md`](design/style.md) 的「四条选择器规则的例外」：**只关最左那一列的左边框**，不动 `--d2h-line-border-color`（改它会连带抹掉行号与代码之间、以及并排两栏之间那两条还有用的竖线）。**修完后 Chrome 复量**（1500×900 走并排 / 1200×900 走 unified）：并排时第一栏行号 `border-left` 为 `0px`、`border-right` 仍 `1px`，第二栏两侧都还是 `1px`（两栏之间那条留住了）；unified 时同样是 `0px` / `1px`；两档截图里侧栏右边界都只剩 1 列 `#e5e5e5`，且补丁表格上下粗细一致。
