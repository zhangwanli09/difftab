...
- **`--` 后面的路径必须按字面量而不是通配模式解释**,封装层统一设 `GIT_LITERAL_PATHSPECS=1`(2026-08-13 于 S4a 补,起因见第 10 节)。路径来自 URL query,是外部输入,而 git 的 pathspec 默认是 wildmatch:`path=*` 会让 `git diff HEAD -- '*'` 回一份**整仓 diff**——正是本节明令禁止的那件事;一个真实存在、名字里带 `*` 的文件同样会匹配到别人身上,页面在 A 的标题下多出 B 的补丁。本项目的路径无一例外来自 git 自己的输出,不需要任何通配语义。**这与「按路径挑 numstat 记录」是两道独立的防线,各自都有只有它才拦得住的形态**,不可相互替代
...
- **兜底**:任一路径失败(ENOSPC / ENOSYS / 网络盘 NFS·SMB / Docker 卷)自动降级为 **1.5s 轮询**,并在 UI 上标注降级模式。这条与档位正交:A / B 档失败时同样落到轮询,C 档则是一开始就以它为工作区通路。`ignore` 解决的是配额,救不了这些场景,**兜底不可省略**
  - **检测得到才降得了级,而 Linux 上的 ENOSPC 有一大半检测不到**(2026-08-12 核对 Node 24.14.1 源码,2026-08-18 在 CI 的 ubuntu runner 上压低配额实测,数据见第 10 节):`internal/fs/recursive_watch.js` 的 `kFSWatchStart` 把**根**那一次注册的失败整个吞掉——`catch (error) { if (error.code === 'ENOENT') throw; }`,ENOSPC / EACCES / EPERM 一律丢弃,`fs.watch()` 返回一个看着活着、却永远不 emit 的 watcher。**实测把这条推得更远:遍历途中耗尽同样不 emit** —— 此前按源码推断它会 `emit('error')` 从而被兜底接住,那个推断**是错的**。真正让 ENOSPC 浮出水面的是**下一次要注册 watch 的时候**,也就是工作区新出现一个条目的那一刻;于是"降级"发生在用户第一次新建文件之后,而不是启动那一刻
...
- **原生档(A / B)同时跑一个低频安全轮询**(`SAFETY_POLL_MS`,30s),用与降级轮询逐字相同的那条 status 命令,只做一件事:发现变化就照常推一次刷新。它补的正是上一条那个缺口——原生监听少报了什么时没有任何信号,只有拿 status 输出本身去比才看得见。**不翻 `mode`、不上报降级**:原生监听确实还活着(只是不完整),翻了会把一次可能的误判说成"已降级",而两者代价不对称——多刷一次没有代价,把状态说错有。**周期取 30s 而不是 1.5s**,是为了让第 6 节"原生监听模式下空闲 CPU 接近零"继续成立(一次 status 几十毫秒,占空比千分之几),代价是那个病态场景下最坏 30s 的滞后。真降级之后周期自动收到 1.5s——两者是同一个轮询循环的两个周期,不是两套机制
...
- **实现要点**:服务端 SSE 心跳约 15s;前端监听 `visibilitychange`,标签重新激活时先按「静默是否超过两拍心跳」判一次死连接,已经不新鲜就掐掉重连。**判死只在这一处做,不另设定时器** —— 用户回来看的那一刻正是最值得重试的时刻;代价是一个**始终可见**的标签页在连接静默后不会自愈,而回环上能造出「连接活着却没人应答」的只有服务进程被冻住这一种形态(已实测,见第 10 节)
...
- [x] `[S3b2/S5]` Linux 上人为压低 `fs.inotify.max_user_watches` 直至触发 ENOSPC 时,能正确降级为轮询并在 UI 提示,功能不受影响(**由专用作业 `inotify-quota` 断言**:GitHub 的 ubuntu runner 有免密 sudo,可以真的把配额压下去)。**它测出的形态与原先设想的不同**:遍历途中的 ENOSPC 一次都不 emit,降级发生在用户第一次新建文件之后,而"改一个启动前就存在的深层文件"则完全静默——后者由 5.7 那条 30s 低频安全轮询兜住,判据是同一作业里那条**改已有深层文件仍刷得出来**的探针(数据见第 10 节)

**进程生命周期与单实例**

- [x] `[S3c/S5]` 页面刷新、系统休眠唤醒、浏览器丢弃后台标签后重新激活,均不导致进程误退出,页面能自动恢复连接(S3c 验到刷新与后台标签两半:刷新 3 次、后台 60 秒、关掉页面 25 秒再开,进程都还在且回来即刷新;**休眠那半 S5 在真机上补齐,并推翻了本项原先给它写的理由** —— 整机休眠在**回环**上不产生半开 TCP,浏览器与服务一起冻结、一起醒来,连接完好。`STALE_MS` 那条路要靠「连接静默 + 标签切回」才走得到,已按该形态实测,见第 10 节)
