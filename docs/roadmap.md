...
- **发布产物约定**:`package.json` 的 `files` 字段白名单为 `bin/`、`dist/`、README、LICENSE;`prepublishOnly` 执行完整构建;发布前用 `pnpm pack --dry-run --json` 核对产物内容,确认不含 `src/`、配置文件与测试(验收见 `acceptance.md` §6)。前端依赖(diff2html / highlight.js / preact)在构建期即被打进 `dist/web/app.js`,后端只用 Node 标准库,因此 **`dependencies` 为空**——用户 `npm i -g` 时零传递依赖安装,应在 README 中说明
