...
- [ ] `[后 0.1.0]` **列表行按「文件名 + 目录」两段展示**(`design.md` §5.4)。顺序、目录不带尾部斜杠、根目录文件不拖空路径段、以及两段同住一个 `truncate` span,四条都由 `test/unit/web/change-list.test.tsx` 断言。**留给人眼的只剩「窄侧栏下先裁的是目录、文件名始终看得见」**——谁被裁掉要排版引擎才判得了。人工判据三条:造一个深目录的长文件名时先没的是目录、深浅两套外观下目录都是次要色且与文件名基线对齐、冲突行的 XY 两位仍在行首且各行文件名左边对齐
