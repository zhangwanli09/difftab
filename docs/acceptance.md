...
- [ ] `[后 0.1.0]` **diff 面板窄到放不下两列时切成逐行视图,拖宽再切回并排**(`design.md` §5.5)。阈值映射与「版式变了会重画」两段由 `test/unit/web/` 的 `layout.test.ts` / `diff-view.test.tsx` 断言,**这条留给人眼的只有 `ResizeObserver` → 宽度 signal 那一段**:happy-dom 的 `ResizeObserver` 是个 `observe()` 什么都不做的空壳、也没有布局引擎(实测见 `decisions.md` §10),CI 里没有任何东西量得出宽度。判据四条:拖窄就地变逐行(不是刷新后才变)、行号列跟着代码滚、在阈值附近来回拖不闪(这条验的是「量 border box 不量 content box」那个取舍)、深色下逐行视图的增删底色仍走我们的 `--d2h-*`
