...
> **本文是记录，不是约束。** 约束在 `AGENTS.md` 第 5 节（红线）与 [`decisions.md`](decisions.md)（被排除的做法）；需求与设计见 [`README.md`](README.md) 的索引。已收口的 0→1 开发阶段记录不在这里，在 git history。
...
- **同一天两次「先建分支再提交」走通**（第九、第十次），`--rebase` 合并在 auto mode 下这两次都没被分类器拦，tag 打在远端那一份 c6b60db 上。
- **registry 延迟第六次，也是最慢的一次**：20 秒一轮，第六轮（约 100 秒）才等到 0.5.2；仍在两分钟内。
- **`check:global` 第六次被上一版验收留下的全局 0.5.1 堵住**，先 `npm rm -g difftab`。
- **发布后验收多加一条**：临时仓库里用发布的 0.5.2 跑 `DIFFTAB_IDLE_MS=1500 difftab --no-open | head -1`，退出码 0——这版修的正是它。其余四条照旧一次过。
...
- **README 刚在 #58 / #59 重写过、未提图片预览，刻意不动**：按 AGENTS.md「特性列表只收差异点」，图片预览不是卖点；发布前清单那条「README 描述的是这版真做的事」只要求不说假话，不要求穷举。
...
- **README 的数字检查这次在发布 PR 之前跑了**，按 0.4.0 那条纠正的顺序：gzip 正文 75KB、`size` 实测 75.0KB；冷启动约 40ms、实测中位 42.8ms，两份 README 一个字没动，没有多出 docs PR，tag 打在版本号提交本身上。
- **registry 延迟第三次遇到，这回量了**：`pnpm publish` 成功后 20 秒一轮的轮询，第六轮（约 100 秒）才等到 0.4.1，`dist-tags.latest` 同步翻过去。0.3.0 记 40 秒、0.4.0 记两分钟内，三次都在两分钟内落地，判据照旧是多等、不重发。
- **`check:global` 第三次被上一版发布后验收留下的全局安装堵住**，先 `npm rm -g difftab` 再跑。每版都命中，说明「发布后」那节装上去的东西就该在「发布前」那节被卸掉——清单已经这样写了，没有新东西要补。
- **「先建分支再提交」第六次走通**，`--rebase` 合并，两侧 tree 哈希相同，tag 打在远端那一份上。
- **发布后四条验收一次过**：`npm view` 回 0.4.1 且 `dist.tarball` 在 npmjs 上、全局装完底下没有传递依赖、在一个临时仓库里 `npx difftab@0.4.1 --no-open` 打印 URL 并按 `DIFFTAB_IDLE_MS` 自行退出、Release 建在 tag `v0.4.1` 上。

### 0.4.0（2026-09-16）

- **minor 号的判据是 0.3.0 那条，不是「有 `feat` 就升 minor」**：本版两条 `feat` 里细滚动条是纯样式（按 0.2.1 的口径归 patch），值一个 minor 的是变更列表每行悬停露出的 `Open file`——它给「打开一个文件」加了第二条入口，且落的是固定 tab 而非预览 tab，属于交互面变了。判断时先按提交 type 机械地报了 0.4.0，回头对 history 才发现口径写在这里而不在 `RELEASING.md`；结论没变，但**下次先读这段再定号**。
- **README 那条数字检查第一次在 tag 之前拦下了东西**：正文 74KB，`pnpm size` 实测 74.7KB（`Open file` 按钮与几处样式加的）。0.3.0 那条写它「靠人记着才会跑」，这回记着了——但跑在发布 PR 合并**之后**，于是多走了一个 docs PR（#48），tag 打在那个 docs 提交（`cd0edef`）上而不是版本号提交上。这没什么不对（pnpm 只查工作树与远端一致），但顺序反了：`RELEASING.md` 的清单里这条本来就在「Before releasing」，**按清单顺序走就不会多一个 PR**。
- **registry 延迟第二次遇到**：`pnpm publish` 成功后第一次 `npm view --registry=https://registry.npmjs.org` 仍回 0.3.0（`dist-tags.latest` 也是），20 秒一轮的轮询在两分钟内等到了，没量精确秒数。0.3.0 那条的结论照旧：多等再下结论、不重发；轮询比盯着看省事。
- **`check:global` 要求全局没装 difftab，而上一版的发布后验收恰好留下一个**：0.3.0 的 `npm i -g difftab` 没卸，本次 `check:global` 前先 `npm rm -g`。`RELEASING.md` 已写着这条，记在这里只是确认它每版都会命中。
- **「先建分支再提交」第五次走通**，两个 PR 均 `--rebase` 合并，两侧 tree 哈希相同，tag 打在远端那一份上。
- **发布后四条验收一次过**：`npm view` 回 0.4.0 且 `dist.tarball` 在 npmjs 上、全局装完底下没有传递依赖、在一个临时仓库里 `npx difftab@0.4.0 --no-open` 打印 URL 并按 `DIFFTAB_IDLE_MS` 自行退出、Release 建在 tag `v0.4.0` 上。
