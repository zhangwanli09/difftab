...
> **TypeScript + Preact + @preact/signals，经 Vite 构建为静态产物**，由后端直接托管。diff 怎么渲染在 [`diff-render.md`](diff-render.md)，配色与层叠隔离在 [`style.md`](style.md)。选型与被排除的做法见 [`../decisions.md`](../decisions.md)，门禁见 [`../gates.md`](../gates.md)。
...
**选中的文件不在新一份列表里了（改动被撤销、被 commit 掉），就连选中态一起清空、右侧回到空态。** 判据是**左栏此刻正在断言这些改动不存在，而右栏还在展示其中一份**——工作区整个变干净时这一点最刺眼：左边写着「Working tree clean」，右边配着一份完整补丁。早先的写法是留着最后那份 diff（「用户刚才在读的东西还在，只是左侧不再高亮」，排除理由见 [`../decisions.md`](../decisions.md)）。
