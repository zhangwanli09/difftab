...
- **`Changes` 那枚图标与状态条上分支名前的是同一枚**（Octicons `git-branch-16`，对应 VS Code activity bar 上的 Source Control），path 由 `BranchStatus` 导出给两处共用、不各抄一份——同 `CODE_COLORS` 从 `ChangeList` 导出给文件树；**`<svg>` 那层外壳同理走共用的 `Octicon`**（理由与 `Badge` 一字不差：两份逐字相同的 JSX 漂开时不报错，只是同一个概念在两处长得不一样）。`Files` 那枚是 `file-directory-16`。
...
- **分支名前面画一枚分支图标**（位置对应 VS Code status bar 最左那枚）。图形取自 **Octicons 的 `git-branch-16`**（MIT），同样**复制 path 数据内联、不装包**，理由与主题开关那三枚一字不差。**detached 与取不到名字时照画**——它标的是「这一栏说的是 HEAD 在哪」，那句话在三种情况下都成立，而按情况藏掉只会让状态条的左边缘忽进忽出。两个类名各挡一件只是「看着不对」的事：`shrink-0` 让 320px 里先被裁的仍是分支名；`self-center` 是因为那一行是 `items-baseline`，而替换元素的基线是它的底边——不写时图标整个坐在文字基线上，比文字高出小半个字。**两个类名都有断言**（`branch-status.test.tsx` 直接钉类名——happy-dom 没有排版引擎，能断言的只有类名在不在），故不进红线。
