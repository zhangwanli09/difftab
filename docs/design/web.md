...
- **顶栏之下是一行两个 tab（`Changes` / `Files`）**，排在错误条之上。位置对应 VS Code 的 activity bar，但**做成一行文字 tab 而不是一列图标**：图标栏要再占一列宽，而 320px 的侧栏里那一列是从文件名身上扣的，两个视图也用不着一整列。默认档是 `Changes`——工具存在的理由仍是「瞥一眼改了什么」，目录树是顺带能做到的第二件事。两个视图**共用中间那一层滚动容器**，不各开一个。
