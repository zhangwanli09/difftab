...
- **选中态是 tab 自己的 `border-b`（选中 `border-editor-foreground`、未选中透明），通栏那条分隔线是栏的 `divider-b`——与侧栏那行 `Changes` / `Files` tab 同一套机制**（`app.css` 里的自定义工具类：`box-shadow: inset 0 -1px var(--color-panel-border)`）。inset 阴影画在栏背景之上、内容之下；tab 外壳被 flex 拉满栏高、自己的 `border-b` 落在 padding box 最后 1px 上，恰好盖在那条线上——活动 tab 是前景色，未选中透明露出底下那条，栏右侧没有 tab 的空白处也是它。**这一栏是这套机制存在的理由**：它是滚动容器（`overflow-x-auto` 之后 `overflow-y` 也算 `auto`），滚动容器按 padding box 裁内容，「栏 `border-b` + tab `-mb-px` 往下压 1px」那种教科书写法在这里伸进 border 带的那 1px 会被整个裁掉——下划线看不见，且留下 1px 纵向可滚溢出，在「始终显示滚动条」的系统设置下就是一条常驻的纵向滚动条；侧栏那行本不是滚动容器，跟着用同一套只是为了全仓库只有一种压线机制。外加 `bg-editor-background` 让活动 tab 与底下的正文连成一片（VS Code 活动 tab 的底色就是编辑器底色）；未选中是次要色 + 悬停底色。类名不与 `App.tsx` 的 `TAB_CLASS` 共用一个常量——内边距与内容都不同，共用之后改一处会连累另一处。
- **栏 `h-9`，与左栏顶栏钉同一个高度**（36px；`app.test.tsx` 拿两处的 `h-*` 类名互相钉住，漂开时只有「看着不齐」）。**栏 `overflow-x-auto scrollbar-none`、每个 tab `shrink-0`**：tab 多了往横向滚，不压扁；`scrollbar-none`（`scrollbar-width: none` + `::-webkit-scrollbar { display: none }`，`app.css` 里的自定义工具类）只是不画滚动条，触控板横滑与 `scrollIntoView` 照常——照 VS Code，标签栏的滚动条不占高度，画出来时栏会被它撑高、与顶栏错开。**活动 tab 要自己滚进视野**（`scrollIntoView`，`inline: 'nearest'`）：新 tab 追加在末尾，溢出之后新开的那个就在屏幕外——正文换了而栏上看不出任何变化，关掉活动 tab 时顶上来的邻居在屏幕外也一样。
...
- **图在面板里水平居中、从顶部起排，放图的那一行（`ImageRow`，flex + `justify-center`）两个视图共用、单张也走它**。只做水平不做垂直——垂直居中要把容器撑到面板高，`FileView` 那层 wrapper 与 diff 侧的 `RenameNotice` 都得跟着改，一张贴顶的图没那么值。**单张那侧不能退成普通块盒**：块盒下 figure 铺满面板宽，caption 的 `self-end` 贴的是面板右边而不是图的右下角，页面上只是「说明文字离图老远」。
