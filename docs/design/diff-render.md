...
- **`colorScheme` 传 `'light'`，不传 `'auto'`**——深浅切换由我们覆写的 `--d2h-*` 承担，理由见 [`style.md`](style.md) 的「为什么 `colorScheme` 传 `'light'`」。

## 文件头整条不显示

diff2html **没有**关掉自带文件头（`.d2h-file-header`）的配置项（上面那四个开关管的是同步滚动、文件列表折叠与吸顶，头照画），这件事只能落在样式那侧的一条 `display: none`（规则本身与它的三处讲究在 [`style.md`](style.md) 的「三条选择器规则的例外」）。
