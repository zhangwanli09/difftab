...
- **量的是面板那一层（`<section>`），不是它里面那层滚动容器**：换格式会改变内容高度 → 竖直滚动条出现/消失 → 那一层的 content box 宽度抖十几个像素，阈值附近于是在两种格式之间来回重画。面板自己不滚（标签栏排在滚动容器之外，见 [`editors.md`](editors.md) 的「标签栏与视图」），它的宽度不随滚动条进出而变。
