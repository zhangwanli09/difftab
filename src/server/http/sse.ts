...
/**
 * 通道只要求这么多 —— `ServerResponse` 满足它。
 *
 * 不直接写 `ServerResponse` 类型是刻意的:通道不该有能力去改状态码、改头、
 * 或者把一条连接当成普通响应结束掉,那些都归调用方(三道校验之后)。
 */
 ...
 * 一条 SSE 消息的线格式。
 *
 * `data` 一律是一行 JSON:多行正文要逐行加 `data: ` 前缀,而正文里一旦出现换行
 * (将来带上文件名之类)就会静默变成半条消息。`JSON.stringify` 保证不含裸换行。
 */
  ...
  const send = (name: string, data: unknown) => {
    ...
    for (const client of clients) {
      try {
         ...
         * `ServerResponse.write()` 对已结束或已销毁的响应不会同步抛,失败是异步的。
         * 这层 catch 只保证一个客户端写炸了不会让这一轮广播半路夭折 —— 后面的连接
         * 照样收得到。
         */
