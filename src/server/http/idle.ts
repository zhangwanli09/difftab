// 空闲退出的计时器：无任何已连接客户端持续 45 秒即退。
//
// 单独一个文件、且把时钟以外的一切都做成注入，是为了让这条规则能被**单测**钉住——45 秒的
// 用例没人会跑第二次，而它要防的两类错都不报错：退早了（用户开着页面，进程没了）与退不掉
// （关完标签留一个后台常驻进程）。计时的**触发**留在调用方：谁算「有客户端」、哪些请求算
// 活动，都是那边的知识。

/** 宽限期。取 30-60s 区间中值。 */
export const IDLE_EXIT_MS = 45_000;

/** 内部环境变量。不是给用户的开关，不进 `--help` 与 README。 */
...
/**
 * 读宽限期。**取值不合法即启动失败，不退回默认的 45s**——与 `DIFFTAB_WATCH_TIER` 同一条
 * 理由：退回时手滑写错的那次照样启动成功、照样跑出一个看着合理的行为，于是「我验过空闲退出
 * 了」建立在一次根本没生效的强制指定上。
 */
...
export interface IdleWatchdog {
  /**
   * 「刚刚有动静」。有客户端就解除计时，没有就重新起一个完整的宽限期。只有这一个方法，是
   * 因为「有人连上了」「有人断开了」「来了个请求」三件事对本模块完全同义——它们唯一的区别
   * 是调用之后 `hasClients()` 答什么，而那是现问的。分成 arm / disarm 两个方法则要求每个
   * 调用点自己想清楚该调哪个，而想错的两种症状（退早了 / 退不掉）都不报错。
   */
  touch(): void;
...
export function createIdleWatchdog(options: IdleWatchdogOptions): IdleWatchdog {
  ...
  return {
    touch() {
      ...
      /**
       * **计时器不是进程活着的理由**，HTTP server 的监听句柄才是。不 unref 的话，close()
       * 之后漏了 stop() 就会把进程多吊住一个宽限期，而那种残留最难看见。
       */
      timer.unref();
    },
    stop() {
