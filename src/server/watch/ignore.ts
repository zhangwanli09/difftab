 ...
 * 路径任一段命中即忽略。
 *
 * `.git` 在集合里**与档位无关**:工作区那条递归 watch 绝不能进 `.git`(一次 gc 就是
 * 几万个条目),`.git` 侧另有目录级非递归 watch 盯着(见 watcher.ts)。
 */
...
/**
 * 大小写归一,对齐 `ignore` 内部的 `nocase: isWindows || isMacOS`。**不导出**:
 * 它只有 `createIsIgnored` 一个消费者,而多一个入口就多一份会跟着漂的契约。
 *
 * Linux 上**原样返回**:那里的文件系统区分大小写,`Node_Modules` 与 `node_modules`
 * 是两个不同的目录,归一等于把一个用户真的想看的目录悄悄屏蔽掉。
 */
