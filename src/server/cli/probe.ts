 ...
 * 正文读到这么多还没完就放弃。
 *
 * 端口可能已经归了一个完全无关的服务,而它的 `/api/instance` 可以是任何东西 ——
 * 一个流式响应会让下面那句 `body +=` 一直涨下去。上限之内,一份真正的
 * `InstanceInfo` 绰绰有余。
 */
  ...
  return new Promise((resolvePromise) => {
    const req = request(
      ...
      (res) => {
        ...
        /**
         * **响应头到手之后,`req` 就不再是错误的出口了**(已在 Node 24.14.1 实测)。
         *
         * 此后 `req.destroy()` 与对端中途断开都只落在 `res` 上(`'aborted'` 随后
         * `'error': ECONNRESET`),而 `IncomingMessage` 会把没人听的 `'error'` 自己
         * 吞掉 —— 于是下面那两处 `destroy()`(超时、正文过大)既走不到 `'end'`、也走
         * 不到 `req.on('error')`,**这个 Promise 永远不 settle**,`start.ts` 里那句
         * `await findLiveInstance()` 就把启动整个吊死,没有任何输出。
         *
         * `'close'` 是 `res` 上唯一覆盖全部结局的事件;正常读完时它排在 `'end'`
         * 之后,那时早已 resolve 过,再 resolve 一次没有作用(首次为准)。
         */
    ...
    /**
     * 超时用 `setTimeout` 而不是 `req.setTimeout`:后者只管**socket 空闲**,
     * 一个每 200ms 吐一个字节的第三方服务能让它永远不触发。这里要的是整次探活的
     * 墙钟上限。
     *
     * **`destroy()` 之后必须自己 resolve**:响应头已经到手时,这一下的错误落在 `res`
     * 上而不是 `req` 上(见上面那段),光 destroy 等于把超时做成了一个不生效的摆设 ——
     * 而症状是启动整个吊死,恰好比没有超时更难看出来。
     */
...
/**
 * 这个仓库已经有活着的实例吗?有就返回它的 URL,没有就 null。
 *
 * 命中之后调用方**不得碰注册表**:那条目是另一个进程写的,连「顺手更新一下」都不行
 * ——它退出时按 pid 只删自己的那条(registry.ts),我们改过的内容会让它留下垃圾。
 */
export async function findLiveInstance(
   ...
   * 错了不会有任何东西报错。归一化复用注册表键那一份实现,理由记在那边。
   */
