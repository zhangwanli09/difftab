...
// 不起子进程的模块:多一次 git 既落在每次 `/api/state` 上,又要往只读白名单
// 里添一条,而读文件存在性一个字节都不写。
 ...
 *   与 `MERGE_MSG` / `AUTO_MERGE`(已实测),而用户处在的是 rebase —— 反过来
 ...
 *:linked worktree 下 `.git` 是个文件、真正的 git 目录在
 * `<主仓库>/.git/worktrees/<名>`,submodule 在 `<父仓库>/.git/modules/<路径>`
 * (均已实测)。拼出来的路径在那两种仓库里永远读不到 —— 于是永远标不出操作,
