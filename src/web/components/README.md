...
界面上那几枚「只画一枚图标」的按钮（顶栏的明暗开关、tab 行右端那枚「全部折叠」）一律走 `IconButton`，理由与 `Badge` 一模一样。它把 `label` 收成必填参数：只画图标时 `aria-label` 是名字的唯一来源、`title` 再给一份 tooltip，摊成两个可选属性时漏掉一个不报错，页面上也什么都看不出来。侧栏那两个 tab 不走它——那两枚另有 `role="tab"` 与选中下划线，是另一种控件。
