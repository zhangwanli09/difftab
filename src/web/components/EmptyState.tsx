...
// 仍是 `Notice` 贴左上——它们在带路径横杠的 `Panel` 里，说的是「这个文件怎么了」而不是「面板
...
/**
 * 没选文件时右侧画的那一块，**两个视图共用，判据只在这里写一次**。
 *
 * 说哪句、配哪枚**按侧栏此刻列的是哪一档定，不按右侧是哪个视图**：切 tab 不动 `activePane`
 * （见 `store.ts`），于是没选文件时右侧一直是 `DiffView`——`FileView` 的空态在产品里压根到不
 * 了：`openFile` 切 pane 与写 loading 在同一个同步 tick 里，重画批到两次写之后，没有一次渲染
