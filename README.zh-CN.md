...
- **文件树**——侧栏第二个 tab 浏览整个仓库，而不只是改动过的那些：已跟踪、未跟踪与被忽略的文件（后者灰显），按目录懒加载。点任一文件在右侧只读查看，高亮复用同一套配色。
...
- **跑起来很轻**——从启动到端口就绪约 40ms，远在 CI 每次提交都验的 300ms 预算之内（九个 Node × 平台组合各跑一遍）；打包后的 JS gzip 72KB。
