// 用例目录布局的门禁。
//
// 守的是一件**只会静默出错**的事：`vitest.config.ts` 按目录分了两个 project(server=node /
// web=happy-dom),include 因此是几条**具体路径**，不是笼统的 `test/unit/**`。于是往
// `test/unit/` 底下（或任何第三个子目录里）放一个用例文件时，vitest 既不报错也不警告——它只是
// **不属于任何 project、压根不会被跑**，而套件照常全绿。反过来也不能把 include 改成一条
// `test/unit/**`：那会把前端用例塞进 node 环境、或把后端用例套上一层 DOM 全局。正确的修法是**新
// 目录要在 vitest.config.ts 里补一个 project**。
//
// **判据直接读 `vitest.config.ts` 的 include，不在这里抄一份目录清单。** 抄清单守不住它自己声称
// 的不变式：`server` 那档的 include 只收 `*.test.ts`，新建一个 `test/unit/server/foo.test.tsx` 同
// 样不属于任何 project，而「目录名在白名单里」这种代理判据是绿的。include 被改窄或某档 project
// 整个删掉，同理。放在 server 那一档是因为它要读文件系统。
 ...
 * `{a,b}`。写成通用 glob 引擎是过度设计，但**必须支持花括号**——否则哪天 include 收成
 * `*.test.{ts,tsx}`，这里会一条都匹配不上，于是全部文件被判成「孤儿」，以一个错误的理由变红。
 */
function globToRegExp(glob: string): RegExp {
  let source = '';
  // charAt 而不是下标：下标在 noUncheckedIndexedAccess 下是 `string | undefined`，而这里的越界读
  // (`i + 2`)是正常控制流的一部分
...
function includePatterns(): string[] {
  ...
  return projects.flatMap((project) => {
    // projects 的类型是个宽联合（还允许字符串与函数形态）。我们只写对象形态，遇到别的形态就让断
    // 言红掉，而不是静默当成「没有 include」
...
it('test/unit 下的每个用例文件都落在某个 vitest project 的 include 里', () => {
  ...
  // 反空断言：遍历或 glob 编译坏掉时上面那条会对着空集合通过——而这道门禁的全部价值就在于别空转
