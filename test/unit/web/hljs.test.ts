// hljs 语言装配的回归测试(spec §5.5)。
//
// 两件事:
//
// 1. **清单本身钉死在 spec §5.5 上。** 下面的 EXPECTED 是手写的字面量,不是从
...
//    该语言的 diff 就此静默不着色。清单是 §5.5 点名的「白名单」与 JS 预算的主导项,
...
/** spec §5.5 逐字列出的 22 个真实语言模块,外加兜底用的 plaintext。 */
const EXPECTED = [
  'bash',
  'c',
  'cpp',
...
describe('hljs 语言装配', () => {
  it('注册结果与 spec §5.5 的清单逐字相等 —— 多一个少一个都失败', () => {
  ...
  it('别名随主模块生效,不需要也不存在单独的模块', () => {
    ...
    // html 属 xml。单独 import 会在构建期 resolve 失败(spec §5.5)。
