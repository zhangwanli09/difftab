...
//
// 与 test/unit/server/{sse,watch}.test.ts 的分工：那边钉通道与 watcher 各自的行为，这边钉「它们
// 真的被接在了一起，而且在三个平台上都成立」——一条 `git checkout -b` 能不能变成浏览器收得到的
// 一个 `change` 事件。两侧全绿而中间没接上是完全可能的：端点忘了 add 进通道、watcher 忘了起、
// `.git` 的路径在 Windows 上对不上，都不报错。
...
test('仓库里 git 写操作之后，SSE 推出一个 change 事件', async () => {
  ...
  try {
    /**
     * 等连接建立**再多等一下**才动手：watcher 是在第一个订阅者到达时才起的（懒起），抢在它前面
     * 写的话事件根本没人在听——而这条用例会以「超时」失败，读起来像监听坏了。
     *
     * **`await connected` 单独不够**：它只说握手那一行到了，而 watcher 是在那之后的
     * `setImmediate` 里才建起来的。C 档尤其躲不过去——它那条 1.5s 轮询的**首拍只建立基线**，抢
     * 在首拍之前切的分支会被算进基线里，于是两头都不响（实测：重构时把这个 sleep 去掉，ubuntu ×
     * Node 22.0.x 那一档随即以 15s 超时变红，而产品代码一个字没动）。
     */
    const sse = openEvents(server.port, server.token);
    await sse.connected;
    await sleep(300);
    /**
     * **对 fixture 仓库的 git 写操作属「开发流程的 git」**：受「零写操作」约束的是产品代码，不是
     * 测试。这里要的就是一次真实的 `.git` 写入，而 `git checkout -b` 写的正是 HEAD。
     */
...
test(`${TIER_ENV} 能把三档逐个强制指定出来`, async () => {
  await setup();
  // 六条档位验收项全压在这个变量上：一台机器只有一个 Node 版本、一个平台，而三档正是按这两者分
  // 的。三档必须给出**三份不同的** watch 取值，否则「我逐档验过了」建立在一次没生效的强制指定上
  const seen = [];
  ...
  assert.deepEqual(seen, [
    { mode: 'native', tier: 'A' },
    { mode: 'native', tier: 'B' },
    // C 档的工作区通路一开始就是轮询——区别必须体现在 mode 上，否则前端的降级标注拿不到判据
    { mode: 'polling', tier: 'C' },
  ]);
});
...
test(`${TIER_ENV}=C：工作区改动经轮询推出 change，且已存在的未跟踪目录里也算数`, async () => {
  await setup();
  /**
   * C 档(Node < 24.14 × Linux)**不建任何递归 watch**，工作区改动只能靠 1.5s 轮询发现。这条用例
   * 同时钉住两件会静默出错的事：
   *
   * 1. 轮询这条通路真的接上了——断了的话页面只是「不刷新」，不报任何错；
   * 2. 轮询用的是**逐字复用**的主查询。写入落在一个**已存在的**未跟踪目录里：漏掉 `-uall` 时 git
   *    把它折叠成一行 `? 未跟踪目录/`，新增文件根本不改变 status 输出，轮询判定「无变化」。
   */
  ...
  try {
    ...
    /**
     * 反复写，而不是写一次然后等。监听是懒起的，而轮询的**首拍只建立基线**：抢在基线之前写的那
     * 一份会被算进基线里，于是「没变化」是对的，用例却以超时失败、读起来像轮询坏了。每拍换一个
     * 新文件名，则无论基线落在哪一刻下一拍都必然不同——与「等一个够长的固定毫秒数」相比，它不
     * 依赖任何一台机器的快慢。
     */
    let n = 0;
    probe = setInterval(() => {
