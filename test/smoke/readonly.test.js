...
//
// 记录手段是 git 自带的 `GIT_TRACE=<绝对路径>`:它把每一次 git 调用连同完整参数追加进日志文
// 件,三端同一套写法。原方案「PATH 上放一个 fake git wrapper」在 Windows 上落不了地 —— Node ≥
// 20.12 起不带 `shell` 就拒绝 spawn `.cmd` / `.bat`,而把 node.exe 复制成 git.exe 时 node 自己
// 会先把 argv 吃掉一截,记到的「完整子命令」是错的。
//
// GIT_TRACE 比 PATH 劫持还多覆盖一层:git **内部**再起的子进程(自动 gc 之类)同样会被记下来,
// 而那正是「写进 .git/ 但不改变 status 输出」的典型 —— 排除「前后 git status 比对」时说的就是
// 它。「零写操作」是产品核心承诺,本文件是它在开发期唯一的自动化护栏。
...
test('劫持真的生效 —— 日志里确实记到了东西', async () => {
  // 这条不是凑数的。GIT_TRACE 没生效(路径不是绝对的、env 没传下去、产品换了个不经封装层的方式
  // 调 git)时,下面那条白名单断言会对着一个**空数组**通过 —— 而假绿的只读门禁比没有门禁更糟
...
test('产品代码里只有两处 child_process:git 封装层与拉起浏览器', () => {
  // 「唯一非 git 子进程豁免」那一半:fake wrapper 也好 GIT_TRACE 也好都劫持不到非 git 的子进程,
  // 只能静态断言。这条与 biome 的 noRestrictedImports 互补而非重复 —— lint 只看 import 说明符,换
  // 个拿到 child_process 的方式就绕过去了,这里扫的是源码字面量
  const allowed = [
  ...
  const walk = (dir) => {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      // 这里**不再**挖空模板字符串:挖空会先把每个 src 文件的每段模板字符串清空再扫,于是写在模板
      // 字符串里的真调用对这道门禁彻底隐形。宁可把代码形状的常量挪出 src/,不要把门禁改钝
  ...
  // 断言的是**相等**而不是「没有多余的」:只查多的那一半时,两个文件双双改名会让白名单静默变空
  assert.deepEqual(
    using.sort(),
    allowed,
