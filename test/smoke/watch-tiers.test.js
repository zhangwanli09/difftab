...
//
// 与别处的分工:`test/unit/server/watch-tiers.test.ts` 钉判档函数、`ignore.test.ts` 钉逐段匹配
// 本身(两者都不碰真实的 `fs.watch`);`test/smoke/events.test.js` 钉「三档能被强制指定出来」与
// C 档的轮询通路;**这里钉的是「在这台机器自动判定出的那一档上,过滤真的拦住了
// `node_modules`」**。
//
// 为什么不强制指定档位:要的是**用户实际会拿到的那一档**,而它由平台 × Node 版本决定 —— 那正
// 是 CI 矩阵的两个维度。强制指定反而验不到:在 Node 22 上强制 A 档拿到的是一个**没有 `ignore`**
// 的递归 watch(Node 对未知选项静默忽略)。
...
/**
 * 批量写入之后、断言「一个事件都没有」之前要等多久。合并窗口是 150ms,这里给到十倍:等不够的话
 ...
 * 这条 CI 泳道**应该**判到哪一档。
 *
 * **不加这条断言的话,判档一旦漂走,覆盖会静默地关掉而不是变红**:两个用例都按拿到的档位分支,
 * 于是 Linux × Node 24 若某天回了 C,test 1 就从 C 那个出口早退、「A 档过滤生效」这条验收项在任
 * 何泳道上都不再被执行,而套件全绿。
 *
 * 这里是刻意把三档表**再写一遍**(`watch/tier.ts` 里那份是唯一实现)—— 本文件跑的是 `dist/` 产
 * 物,断言的是「发布出去的那份跟约定一致」,复用实现就把这层意思抵消了。只按 major.minor 比:
 * CI 上跑的都是正式版,预发布标签那条边界归单测。
 */
...
/**
 * 进程 `pid` 当前持有的 inotify watch 数(Linux)。判据是 `/proc/<pid>/fdinfo/<fd>` 里的
 * `inotify wd:` 行:一行一个 watch。**不能只数 inotify fd 的个数** —— Node 在 Linux 上的递归实
 * 现是用户态遍历、每个目录一个独立的 `fs.watch`,而 `.git` 侧那几条非递归的也各占一个 fd,两种
 * 形态混在一起就再也分不开了。
 */
...
test('自动判定的那一档:node_modules 深层批量写入不刷新,同一轮里仓库内的新文件必须刷新', async (t) => {
  ...
  try {
    /**
     * **先连 SSE 再取 `/api/state`**:监听是懒起的,连之前那一次拿到的 `mode` 只是
     * `initialMode(tier)` 给的既定值,拿它断言等于断言了一个常量。`sleep` 是给 watcher 一点起身
     * 的时间,抢在它前面写等于对着没人听的通道写。
     */
    ...
    /**
     * **C 档跳过,这是已知边界而不是漏测**:C 档的工作区通路是轮询,比的是 `git status` 的输出
     * 本身,那条路上根本不调 `isIgnored` —— 没有 `.gitignore` 的仓库里,`node_modules` 下的新文
     * 件照样进列表。它的验收项是另一条(inotify 用量),见下一个用例。
     */
    if (tier === 'C') {
...
test('读 /api/state 不会引出刷新事件 —— 自激循环的判据', async (t) => {
  await setup();
  /**
   * 每读一次状态就跑一次 `git status`,而**只要它往 `.git` 里写一个字节,`.git` 侧的 watch 就会
   * 推一个 `change`,前端收到就再读一次状态** —— 一个不报错、只是 CPU 常年挂着 1% 的自激循环,
   * 而 status 的输出从头到尾都是对的。挡住它的是封装层那句 `GIT_OPTIONAL_LOCKS=0`。
   *
   * 那条红线原本只有第二层 B 半的逐字节快照盯着,而**那是在单进程里比对文件,不经过监听**。这
   * 里从另一头断:连着 SSE 的时候连读几次状态,一个事件都不该冒出来。
   */
  const server = await startDifftab({ cwd: repos.staged });
  try {
    const sse = openEvents(server.port, server.token);
...
test('Linux · inotify 用量:A 档不随 node_modules 的目录数增长,C 档只有 .git 侧那几条', async (t) => {
  ...
  if (process.platform !== 'linux') {
    // inotify 是 Linux 专有;macOS 与 Windows 都是单句柄监听整棵树,本就没有配额这回事
    t.skip('inotify 是 Linux 专有');
    return;
  }

  /**
   * 单独造一个仓库,不复用上一个用例那个:那边的 `node_modules` 只有一条深路径,而这里要的正是
   * **目录数**本身 —— 过滤失效时涨的是它。
   */
  const big = makeFixtures(join(workdir, 'big'), ['staged']).staged;
  const PACKAGES = 200;
  for (let i = 0; i < PACKAGES; i += 1) {
    // **只建目录、不放文件**:递归 watch 照样要逐个注册(判据是目录数),而 `git status` 看不见
    // 空目录;这个 fixture **没有 `.gitignore`**,放了文件就会有 200 条未跟踪路径进每一次 status
  ...
  try {
    ...
    /**
     * **`/api/state` 必须在 SSE 连上之后才取**:监听是懒起的,连之前那一次拿到的 `mode` 只是
     * `initialMode(tier)` 给的既定值,拿它断言等于断言了一个常量。
     */
    const state = JSON.parse((await authedGet(server.port, server.token, '/api/state')).body);
    const { tier, mode } = state.watch;
    ...
    /**
     * A 档:`ignore` 在 Linux 上是**注册前跳过**,于是 `node_modules` 那 400 个目录一个都不该进
     * 来。上限取 24(仓库根 + 5 个文件 + `.git` 侧 4 个,留一倍余量),同时压一条与目录数挂钩的
     * 相对判据 —— 阈值将来因别的原因放宽时,后者仍拦得住「过滤失效」这一种。
     */
    assert.equal(tier, expectedTier(), '判到的档位与三档表对不上');
    /**
     * **「数得低」与「还在原生监听」必须一起断**:递归 watch 若因任何原因没建起来(配额耗尽、网
     * 络盘 ENOSYS、根那次注册失败),数出来同样是 `.git` 侧那几条 —— 三条上限断言**全部通过**,
     * 而结论会写成「过滤生效」。`mode` 是把这两种情形分开的唯一判据。
     */
