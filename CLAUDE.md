...
**当前进度:S1 实现完成,待 CI 绿后打勾收口** —— CLI + HTTP server(§5.9 三道校验最终形态)+ 注册表写入 + git 封装层 + §5.12 协议类型 + fixture 第一批 + §5.10 主门禁 + 冷启动门禁接真实流程,本机 `biome ci` / `typecheck` / 67 个单测 / 25 个冒烟 / bench 全绿。**spec §6 的 `[S1]` 验收项待 CI 绿后再勾**(口径见下:本机绿不算)。**每收口一个阶段回来改这一行。**
