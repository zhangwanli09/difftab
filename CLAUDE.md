...
| 开发模式(Vite dev server + 后端) | 先在另一个终端 `node bin/gitglance.js --no-open`,再 `pnpm dev` —— dev proxy 在**加载时**从 `os.tmpdir()` 的注册表读 port + token,后端重启后 dev server 也要重启 |
...
**当前:S3b2 已收口(CI 全绿,run `31597447852` 11 个作业),下一步 S3c。**

- 体积 JS 201.4 KB / gzip 67.5 / CSS 28.5 KB;冷启动中位 42.6ms
- spec §6 的 1 个 `[S3b2]`(资源占用)已勾;6 条 `[S3b2/S5]` 单机只验得到前一半,按规矩不勾

**未消费的跨阶段交接**——消费掉即从本节删除,连同该阶段的收口记录一起落到 `docs/journal.md`。

- **→ S3c(三条)**:① 轮询**不看有没有订阅者**,关掉最后一个标签页之后它照样每 1.5s 起一次 `git status`(C 档,或任一档降级之后),`change` 也照样往空连接集合里推——§5.8 的空闲 45 秒退出把它压成一个 45s 的上界,真要提前掐,判据是 `events.size`;② 轮询与合并窗口的定时器都 `unref()` 过、**不会吊住事件循环**,空闲退出因此不必先去停监听,但 `close()` 里「先停监听与心跳、再断连接」的顺序不能反过来;③ header 现在是**一个 guard 包住两项**(`BranchStatus` + `WatchBadge`),S4b 的 `operation` 标注直接加在同一个 guard 里,别再回到每项各写一次 `state !== null`
