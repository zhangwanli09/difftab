...
**当前:S3c 代码与门禁就位、本机全绿(单测 193 / 冒烟 45 / 冷启动中位 42.6ms 无变化),但 CI 尚未跑过——按本节规矩打勾以 CI 绿为准,所以 spec §6 那 3 个 `[S3c]` 仍空着,收口记录也还没落 `docs/journal.md`。CI 绿之后再收口,下一步 S4a。**

- 体积不变(JS 201.4 KB / gzip 67.5 / CSS 28.5):S3c 只动后端与 CLI
- 本机浏览器实测:刷新 3 次、后台标签 60s、关掉页面 25s 再开,进程都还在且回来即刷新;关掉最后一个标签约 45s 后自己退、注册表条目一并清掉
...
- **→ S5(一条)**:Linux 上「启动时 inotify 配额已被别的工具耗尽」这一种 ENOSPC **检测不到**——`kFSWatchStart` 把根那次注册的失败整个吞掉(只有 ENOENT 会抛),`fs.watch()` 返回一个看着活着、永远不 emit 的 watcher,于是 `mode` 停在 `native`、工作区从此静默不刷新;遍历**途中**耗尽则照常抛得出来、兜底生效。两条候选补法(低频安全轮询 / 建流前先探一次非递归 watch)**择一的判据留给 S5 在真机上压低 `max_user_watches` 实测**,现在不猜。已写进 spec §5.7 与 §10,§6 那条验收项本就带 `/S5`
