...
//
// 三项断言:
//   1. 该文件在 HEAD 与 index 两侧都以 100755 入库(可执行位),理由见下方 assertExecutableBit;
//   2. 跑完完整构建后,该文件与构建前逐字节一致;
//   3. 守卫的特征串没有出现在 dist/ 的任何产物里 —— 若它被当作打包入口
 ...
 * 发出去的那份始终是对的。所以这里钉的是**仓库自己记的 mode**,不是 tarball 里的 mode。
 *
 * **HEAD 与 index 两侧都要查,只查 index 是不够的**:`git update-index --chmod=+x` 只写
 * 暂存区,而 fresh clone 拿到的是 HEAD 那份。只查 index 时,一个「chmod 过、看见 PASS、
 * 但那次改动始终没进提交」的本机就绿了,别人 clone 下来照样踩原来的坑 —— 而 CI 上
 * index 恒等于 HEAD,于是这道门禁会恰好在它唯一要保护的地方(开发者本机)最弱。
 */
